...
console.log('\n[ master v5.2 ]');
check(master.version === 'v5.2' && master.language === 'en', 'master_cv is v5.2 / English');
...
console.log('\n[ CV structure + claims ]');
check(hair.source_master === 'master_cv@v5.2', 'CV traces master_cv@v5.2');
...
check(!/Microblading|Permanent make-up|Advanced esthetics|Event styling/i.test(coreSkills), 'Core Skills excludes training-only skills');
check(/Microblading/.test(hair.content) && /Permanent Make-up/.test(hair.content), 'microblading/permanent make-up still present under Additional Training');
...
const T = {
  sukhi: "Your posting calls for colour work — applying bleach, tints, and rinses — alongside cutting and styling, which are services I perform daily. It also asks for suggesting a style that suits each client's features. I would be glad to bring this colour and styling focus to Sukhi Laser Beauty Salon & Academy Ltd.",
  blades: "Your posting spans a broad range of hair services, and my experience aligns with the women's hairdressing side — cutting, colouring, and treatments. My background is in women's hairdressing rather than barbering. I keep these techniques current through ongoing training.",
  oliha: "Your posting includes booking appointments and supervising other stylists — responsibilities I carry out every day as owner-manager of my own salon. I manage scheduling and coordinate the team while cutting, colouring, and styling hair myself. I would bring both to OLIHA MUNIZ BOUTIQUE AND HAIR INC.",
  glam: "As a hairdresser and esthetician, I provide general esthetic care and make-up with careful consultation and hygiene. Running my own salon, I manage a full appointment schedule while delivering services myself. I would welcome the chance to learn more about Glamour Touch Studio Inc.",
};
...
// perming/waving unsupported -> absent from CV and every letter
check(!/\bperming\b|permanent wave/i.test(hair.content) && !/\bperming\b/i.test(esth.content), 'CV omits perming/waving');
[a, b, o, e].forEach((L, i) => check(!/\bperming\b|permanent wave|\bperm and\b/i.test(L.content), `letter ${i + 1} omits perming/waving`));
// esthetician letter must NOT claim facials/wax/nails/lashes/laser
check(!/facial|wax|\bnail|lash|laser|device|medical/i.test(e.content), 'esthetician letter omits facials/wax/nails/lashes/devices');
